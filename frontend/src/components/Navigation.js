...
const Navigation = ({ stage, setStage, handleLogout, goHome, goToBlog }) => {
    const navItems = [
        { name: 'Home', icon: <Home className="w-5 h-5" />, stage: 'dashboard', action: goHome },
        { name: 'Adventures', icon: <BookOpen className="w-5 h-5" />, stage: 'adventures', action: () => setStage('adventures') },
        { name: 'Blog', icon: <FileText className="w-5 h-5" />, stage: 'blog', action: goToBlog },
    ];
    ...
    return (
        <div className="fixed top-0 left-0 h-full w-20 bg-card flex flex-col items-center py-4 shadow-lg border-r border-border z-30">
            <div className="flex flex-col items-center justify-between h-full w-full">
                ...
                {navItems.map(item => (
                        <button key={item.name} onClick={item.action} className={`flex flex-col items-center justify-center gap-1 p-2 rounded-lg transition-colors w-16 ${stage === item.stage ? 'text-primary bg-primary/10' : 'text-muted-foreground hover:text-foreground hover:bg-accent'}`}>
                        {item.icon}
